...
public class VideoSegmentationOp {
    ...
    ///////// PARTE 3: COMPARISON AND ALLOCATION OF DATA  ////////////////////////////////////
    ...
    ///////// PARTE 4: IDENTIFICATION OF KEY_SCENES AND KEY_FRAMES  ///////////////////////////////////////
       ...
       // System.out.println(count_frames_each_scene);  
        ...
        List<Integer> Key_Escenes = new ArrayList<>();                                                                  // Create a list to store the indices of dense scenes (dense = +20 frames, we avoid transitions)
        ...
        for (int i = 0; i < final_index; i++) {
            if (count_frames_each_scene.get(i) >=  (25/range) ) {                                                  // Check if the cluster size is at least 25, to avoid frames that are transitions, which are not considered KeyFrames
               Key_Escenes.add(i);                                                                                    // Add the cluster index to the list of dense clusters
            } 
        }

       // System.out.println(Key_Escenes);                                                                              // Print the indexes of the video scenes (those that have more than 20 frames)
        ...
        for(int i=0; i<final_index;i++){          
            ...
            for(int j=0; j<Key_Escenes.size();j++){
                if(i == Key_Escenes.get(j)){                                                                       // When a scene considered key (dense) is found, its penultimate frame is saved as KeyFrame
